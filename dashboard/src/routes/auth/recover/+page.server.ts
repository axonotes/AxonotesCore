...
export async function load({locals, cookies}) {
    ...
    try {
        ...
        // If user has no backup key, they haven't completed setup.
        if (!currentUser?.encryptedBackupKey || !currentUser.publicKey) {
            throw redirect(302, "/auth/setup");
        }

        // The backup key is stored as a JSON string in the DB.
        const encryptedBackupKey: EncryptedData = JSON.parse(
